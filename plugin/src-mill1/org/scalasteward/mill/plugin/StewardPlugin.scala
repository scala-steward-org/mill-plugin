...
object StewardPlugin extends ExternalModule {
  ...
  def extractDeps(ev: Evaluator) = {
    val modules = Task.traverse(findModules(ev))(toModuleDep)
    Task.Command {
      val m = modules()
      Obj(
  ...
  private type WithRootModule = {
    def rootModule: api.internal.RootModule0
  }

  def findModules(ev: Evaluator) =
    mill.scalasteward.internal.getRootModule(ev).moduleInternal.modules.collect { case j: JavaModule => j }

  def toModuleDep(m: JavaModule): Task[ModuleDependencies] = {
    val dependencies = Task.Anon {

      val convert = plainCoursierDep(m)()

      val mvn = m.mvnDeps() ++ m.mandatoryMvnDeps() ++ m.compileMvnDeps() ++ m.runMvnDeps()
  ...
  private def plainCoursierDep(module: CoursierModule): Task[Dep => coursier.Dependency] =
    Task.Anon { (dep: Dep) =>
